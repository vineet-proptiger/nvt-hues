        ...
        <div className="mb-12 text-center" data-aos="fade-up" data-aos-duration="1000">
          ...
          <div className="flex justify-center items-center">
            <button 
              ...
              className={`px-8 py-3 text-[17px] font-medium transition-all cursor-pointer border border-[#3D5915] ${activeTab === 'master' ? 'bg-[#3D5915] text-white' : 'bg-transparent text-[#3D5915]'}`}
            >
              Master Plan
            </button>
            <button 
              onClick={() => setActiveTab('floor')}
              className={`px-8 py-3 text-[17px] font-medium transition-all cursor-pointer border border-l-0 border-[#3D5915] ${activeTab === 'floor' ? 'bg-[#3D5915] text-white' : 'bg-transparent text-[#3D5915]'}`}
            >
              Floor Plan
            </button>
          </div>
            ...
            <div className="w-full max-w-[820px] mx-auto bg-white rounded-md overflow-hidden shadow-[0_5px_15px_rgba(0,0,0,0.1)] p-4" data-aos="zoom-in" data-aos-duration="1000">
              <a onClick={() => setIsOpen && setIsOpen(true)} className="cursor-pointer block relative overflow-hidden group bg-[#fdfbf7]">
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-white/20">
                  <span className="bg-[#3D5915] group-hover:bg-[#6E942A] text-white px-7 py-4 text-[14px] sm:text-[15px] leading-snug font-semibold uppercase tracking-wider text-center shadow-md transition-colors duration-300">
              ...
              {plans.map((plan, idx) => (
                <div key={idx} className="bg-white rounded-md overflow-hidden shadow-[0_5px_15px_rgba(0,0,0,0.1)] transition-transform hover:-translate-y-1" data-aos="fade-up" data-aos-duration="1000" data-aos-delay={idx * 100}>
                  <a onClick={() => setIsOpen && setIsOpen(true)} className="cursor-pointer block relative h-[260px] overflow-hidden group bg-[#fdfbf7] flex items-center justify-center p-4">
                    ...
                    />
                    <div className="absolute inset-0 flex items-center justify-center bg-white/20">
                      <span className="bg-[#3D5915] group-hover:bg-[#6E942A] text-white px-5 py-3 text-[13px] leading-snug font-semibold uppercase tracking-wider text-center transition-colors duration-300">
